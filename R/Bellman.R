  ...
  #' @param Rewards for each simulation value and each Monte-Carlo year.
  ...
  #' @param correct_outliers If TRUE, outliers in Bellman values are replaced by spline
  #'   interpolations. Defaults to FALSE.
  #' @return a \code{Numeric} the Bellman values
  #' @importFrom stats ave quantile
  #' @export


  Bellman_value <- function(states,statesID,level_high,level_low,value_inflow,
                            reward, states_next,next_week_values_l,decision_space,E_max,
                      niveau_max,method,na_rm=TRUE,max_mcyear,print_test=FALSE,
    ...
    for (l in decisions) { # l <- 0
      ...
      # Bellman value of the next week
      vunw <- next_week_values_l[next_node_up]
      # if (!is.finite(vunw))
      #    vunw <- 0
      vdnw <- next_week_values_l[next_node_down]
      # if (!is.finite(vdnw))
      #   vdnw <- 0
      interpolation <- remainder * vunw + (1 - remainder) * vdnw
  ...
  Bellman_parallel <- function(Data_week,next_week_values_l,decision_space,E_max,
                      niveau_max,method,na_rm=TRUE,max_mcyear,print_test=FALSE,
                      correct_outliers=FALSE,q_ratio=0.75,test_week,counter,inaccessible_states=F,...){
...

    {
      ...
      bellman_values <- foreach::foreach (i=1:nrow(Data_week), .combine = 'c',
            ...
            bellman_parallel_value(Data_week,i,next_week_values_l,max_mcyear,
                                   E_max,niveau_max,na_rm,decision_space,alpha,decimals)
...
#' @export
  ...
  #init
  {
    ...
    next_week_values <- next_week_value(next_week_values_l,max_mcyear,
                                        nrow=nrow(Data_week),i)
...
next_week_value <- function(next_week_values_l,max_mcyear,nrow,i){

    ...
    for (n_cycl in seq_len(nb_cycle)) {
      ...
      for (i in rev(seq_len(52))) { # rep(52:1, times = nb_cycle)
        ...
        temp <- Bellman(Data_week=temp,
                        ...
                        penalty_level_low=if((i==52)&(n_cycl==1)){penalty_final_level_low}else{penalty_low},
                        penalty_level_high=if((i==52)&(n_cycl==1)){penalty_final_level_high}else{penalty_high},
                        lvl_high =temp$level_high[1],
                        lvl_low =temp$level_low[1],
      ...
      value_nodes_dt <- build_data_watervalues(watervalues,statesdt,reservoir,penalty_high,penalty_low,
                                               if(n_cycl==1){penalty_final_level_low}else{penalty_low},
                                               if(n_cycl==1){penalty_final_level_high}else{penalty_high})

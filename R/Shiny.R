...
#--------post process----------
                        ...
                        spsUtil::quiet({
                          spsComps::shinyCatch({
                            results <-  calculateBellmanWithIterativeSimulations(
                              ...
                              q_ratio = input$q_ratio_dp/100)$results
    ...
    shiny::observeEvent(input$itr_to_antares,
                        {reshaped_values <- rv$itr_results[rv$itr_results$weeks!=53,] %>% to_Antares_Format(input$itr_penalty_low,input$itr_penalty_high)
                        antaresEditObject::writeWaterValues(
                          area = input$itr_sim_area,
                          data = reshaped_values
                        )

...
#'
#' Generate a list of control values, eg difference between final and initial level for each week for the given area.
...
#' @param nb_disc_stock Integer. Number of constraint values wanted for each week.
#'
#' @return A \code{dplyr::tibble()} with 3 columns : \code{"week"} (integer), \code{"u"} (double), \code{"sim"} (character).
#' For each week from 1 to 52, \code{nb_disc_stock} different controls values are generated from maximum pumping power to maximum generating power.
#' The controls are equally spaced between maximum pumping power (negative value) and 0 and between 0 and maximum generating power (positive value).
...
constraint_generator <- function(area,nb_disc_stock,pumping=TRUE,efficiency=NULL,opts,max_hydro_weekly=NULL,
  ...
  max_hydro_weekly <- dplyr::left_join(max_hydro_weekly,max_app,by=c("timeId"))
...
#'
#' @param pumping Boolean. True to take into account the pumping.
#' @param efficiency between 0 and 1. the pumping efficiency ratio.
#' @param nb_disc_stock Number of constraint values wanted for each week
#' @param reservoir_capacity Double, reservoir capacity
...
#' @keywords internal
constraint_week <- function(pumping,efficiency,nb_disc_stock,reservoir_capacity,hydro,week){
  maxi <- min(hydro$turb,reservoir_capacity+hydro$max_app)
  mini <- max(-reservoir_capacity,-hydro$pump*efficiency)

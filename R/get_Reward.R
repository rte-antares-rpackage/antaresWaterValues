...
get_Reward <- function(simulation_values = NULL,simulation_names=NULL, pattern = NULL,
  ...
  if(method_old){
    ...
    {reward <- lapply(
      X = opts_o,
      FUN = function(o) {
        res <- get_weekly_cost(mcyears = mcyears, opts = o, expansion = expansion)
    ...
    {reward <- mapply(
      FUN = function(o,u) {
        res <- get_local_reward(o,possible_controls,max_hydro,area,mcyears,u[[1]],
                                  pump_eff)
        res <- reward_offset(o,res, u[[1]],mcyears, expansion=expansion)
...
#' @return a data.table {mcYear,week,u,reward}
#' @export
reward_offset <- function(opts, df_reward, u0=c(),mcyears,
                          expansion = F){
  cost <- get_weekly_cost(mcyears = mcyears, opts = opts,
                          expansion=expansion) %>%

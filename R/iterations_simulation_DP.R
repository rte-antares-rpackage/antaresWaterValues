...
calculateBellmanWithIterativeSimulations <- function(area,pumping, pump_eff,opts,
                                                     nb_control,nb_itr,mcyears,
  ...
  while(gap>1&i<=nb_itr){
    ...
    expected_reward <- getExpectedReward(df_rewards=df_rewards,levels=levels)
...
getExpectedReward <- function(df_rewards,levels){
  expected_reward <- df_rewards %>%
    dplyr::group_by(.data$mcYear,.data$week,.data$u) %>%
    dplyr::summarise(reward=min(.data$reward),.groups="drop") %>%

...
This article explains the different parameters on the function \code{Grid_Matrix()} and how to use them.
...
The parameter \code{states_step_ratio} can be decreased to have more precise results but this will also increase computational time.
...
Bellman values can be computed with 3 different formulas : - \code{grid-mean} : At the end of each week in the backaward recursion, for each state compute the mean of Bellman values over the scenarios. This method computed the minimum of the espenctancy of weekly costs with the constraint that the hazards of the following week are not yet known. This approach is the most accurate mathematically speaking. Use this method to minimize costs. - \code{mean-grid} : Compute the mean of Bellman values at the end of the backward process. - \code{quantile} : Similar to \code{grid-mean} but instead of the mean, take the quantile. Use this method to avoid years with high costs and in particular to minimize the unsupplied energy. The value \code{cvar_value} can be adapted to be more or less cautious.

```{r compare_method, message=FALSE}
compare_method = data.frame()
for (m in c("mean-grid","grid-mean","quantile")){
...
The \code{quantile} method gives higher water values and therefore more conservative levels trajectories.
...
Bellman values with \code{quantile} method are smaller as they take into account for each week, the worst scenarios so they overestimate the total cost.

# Respect of rule curves

In case, there is rule curves in the storage, penalties on bottom and top rule curves can be used to encourage storage levels to stay inside rule curves. The value of \code{penalty_low} (in euros/MWh) should be chosen with respect to thermal clusters cost and to unsupplied cost, depending if it is better to use expensive thermal cluster or to not respect the lower rule curve. On the opposite, \code{penalty_high} should be chosen with respect of the spillage cost.
...
As for the respect of rule curves, it is possible to encourage the storage to end the year at a specific level thanks to penalties. The option \code{force_final_level} must be \code{TRUE}. The wanted final level is specified with parameter \code{final_level}. Specific values of penalties are used for the final week and are given by \code{penalty_final_level_low} and \code{penalty_final_level_high}.

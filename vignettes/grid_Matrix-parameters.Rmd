...
-   `quantile` : Similar to `grid-mean` but instead of the mean, take the quantile. Use this method to avoid years with high costs and in particular to minimize the unsupplied energy. The value `cvar_value` can be adapted to be more or less cautious.
...
Bellman values with `quantile` method are smaller as they take into account for each week, the worst scenarios so they overestimate the total cost.
